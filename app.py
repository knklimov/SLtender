...
def process_excel(data_file, plan_file):
                    ...
                    if color and isinstance(color, str) and color.upper() != TARGET_GREEN:
                        try:
                            ...
                            if isinstance(original_value, (int, float)) and isinstance(min_value, (int, float)):
                                new_value = max(original_value - 3, min_value)
...
st.title("Редактирование Excel по цвету с учётом индивидуальных минимумов")
...
if plan_file and data_file:
    processed_file, log = process_excel(data_file, plan_file)
